...
func TestSimpleNetwork(t *testing.T) {
	w, err := mat.NewMat64(2, 3, vec.vec{
	...
	x, err := mat.NewMat64(1, 2, vec.vec{0.6, 0.9})
	if err != nil {
		panic(err)
	}
	...
	expectedPredict, _ := mat.NewMat64(1, 3, vec.vec{
	...
	ta, _ := mat.NewMat64(1, 3, vec.vec{0, 0, 1})
...
func calcNetwork(wvec vec.vec) float64 {
	w, err := mat.NewMat64(2, 3, wvec)
	if err != nil {
		panic(err)
	}
	x, err := mat.NewMat64(1, 2, vec.vec{0.6, 0.9})
	...
	sn := NewSimpleNet(w)
	t, _ := mat.NewMat64(1, 3, vec.vec{0, 0, 1})
	return sn.Loss(x, t)
}
...
func TestSimpleNetworkGradient(t *testing.T) {
	...
	expected, _ := mat.NewMat64(2, 3, vec.vec{
